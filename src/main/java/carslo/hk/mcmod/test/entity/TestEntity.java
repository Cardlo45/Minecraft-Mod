...
public class TestEntity extends Hk400testModElements.ModElement {
	...
	public void initElements() {
		...
		elements.items
				.add(() -> new SpawnEggItem(entity, -16777216, -1, new Item.Properties().group(ItemGroup.MISC)).setRegistryName("test_spawn_egg"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			biome.getSpawns(EntityClassification.AMBIENT).add(new Biome.SpawnListEntry(entity, 4, 1, 2));
		}
	...
	public static class Modelcustom_model extends EntityModel<Entity> {
		...
		public void setRotationAngles(Entity e, float f, float f1, float f2, float f3, float f4) {
			...
			this.Wing0.rotateAngleZ = MathHelper.cos(f * 0.6662F) * f1;
		}
	}
}
