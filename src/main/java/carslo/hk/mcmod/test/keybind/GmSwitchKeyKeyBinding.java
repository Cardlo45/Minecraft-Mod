...
package carslo.hk.mcmod.test.keybind;
...
import carslo.hk.mcmod.test.procedures.GmSwitchPressProcedure;
import carslo.hk.mcmod.test.procedures.CloseGuiProcedure;
import carslo.hk.mcmod.test.Hk400testModElements;
import carslo.hk.mcmod.test.Hk400testMod;

@Hk400testModElements.ModElement.Tag
public class GmSwitchKeyKeyBinding extends Hk400testModElements.ModElement {
	@OnlyIn(Dist.CLIENT)
		...
		if (Minecraft.getInstance().currentScreen == null) {
			if (event.getKey() == keys.getKey().getKeyCode()) {
				if (event.getAction() == GLFW.GLFW_PRESS) {
					Hk400testMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(0, 0));
					pressAction(Minecraft.getInstance().player, 0, 0);
					lastpress = System.currentTimeMillis();
				} else if (event.getAction() == GLFW.GLFW_RELEASE) {
					int dt = (int) (System.currentTimeMillis() - lastpress);
					Hk400testMod.PACKET_HANDLER.sendToServer(new KeyBindingPressedMessage(1, dt));
		...
		if (type == 0) {
			{
				...
				GmSwitchPressProcedure.executeProcedure($_dependencies);
			}
		}
		if (type == 1) {
			{
